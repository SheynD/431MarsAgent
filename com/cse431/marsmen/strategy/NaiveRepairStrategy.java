...
public class NaiveRepairStrategy implements Strategy {
	...
	public Action execute(MarsAgent agent) {
		...
		for (LogicBelief visibleEntity : agent.getAllBeliefs("visibleEntity", agent.getName())){
			System.err.println("VisibleEntity: " + visibleEntity.getParameters().get(0) + " " + visibleEntity.getParameters().get(1) + " " + visibleEntity.getParameters().get(2) + " " + visibleEntity.getParameters().get(3) + " " + visibleEntity.getParameters().get(4));
			System.err.println("agent: " + agent.getName() + " " + agent.getTeam());
			for (String l : u.getNeighborVertexes(agent.getAllBeliefs("position", "", agent.getName()).getFirst().getParameters().get(0))){
			...
			if (agent.getTeam().equals(visibleEntity.getParameters().get(3)) && /* If they are on out team */
					...
					u.getNeighborVertexes(agent.getAllBeliefs("position", "", agent.getName()).getFirst().getParameters().get(0))
					.contains(visibleEntity.getParameters().get(2))){ /* And they are a neighbor */
				//System.err.println(visibleEntity.getParameters().get(0) + " " + visibleEntity.getParameters().get(1) + " " + visibleEntity.getParameters().get(2) + " " + visibleEntity.getParameters().get(3) + " " + visibleEntity.getParameters().get(4));
